...
function calcPuntos(pron, partido) {
  ...
  const gl = Number(pron.goles_local),
    gv = Number(pron.goles_visitante);
  const rl = Number(partido.goles_local_real),
    rv = Number(partido.goles_visitante_real);
...
export default function RankingJornada({ user }) {
  ...
  const loadJornada = async (j) => {
    ...
    // Puntos máximos adicionales = 3 × partidos sin resultado
        ...
        <>
          ...
          {hayPendientes && tieneResultados && (
            <div className="rj-leyenda">
              <div className="rj-ley-item">
                <span className="rj-dot rj-dot-zona" />
                Zona dinero (top 3)
              </div>
              <div className="rj-ley-item">
                <span className="rj-dot rj-dot-con" />
                Con posibilidades
              </div>
              <div className="rj-ley-item">
                <span className="rj-dot rj-dot-sin" />
                Sin posibilidades
              </div>
            </div>
          )}
            ...
            {tabla.map((u) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`rr-avatar ${chances === "zona" ? "rr-avatar-zona" : chances === "sin" ? "rr-avatar-sin" : ""}`}
                  >
